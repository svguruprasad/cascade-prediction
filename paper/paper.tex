...
We make this concrete with direct measurement. In a controlled experiment on AWS (us-east-1), we set Lambda reserved concurrency to 5 and sent 200 concurrent requests. Result: 86\% throttle rate, 1,248 downstream operations across DynamoDB, Kinesis, and CloudWatch that never occurred. CloudWatch independently confirmed 312 throttle events.
...
\begin{enumerate}
  \item Direct measurement of cascade amplification factors from a live 3-function contact flow on AWS: 1 call produces 3 Lambda invocations generating 9.5$\times$ total downstream operations (2.0$\times$ DynamoDB writes, 1.5$\times$ reads, 3.0$\times$ Kinesis records, 3.0$\times$ CloudWatch puts). Confirmed across two independent runs.
...
We measured amplification factors from a 3-function contact flow on live AWS infrastructure (90 calls, 2 runs, CloudWatch-confirmed):

\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{figures/fig1_dependency_graph.png}
\caption{Measured service dependency graph. Solid red lines: always executed (Lambda 1 and 3). Dashed orange lines: conditional (Lambda 2, 60\% of calls). Numbers on edges are measured amplification factors confirmed by CloudWatch across two independent runs.}
...
\begin{tabular}{@{}lccc@{}}
\toprule
Operation & Per Call & CloudWatch Metric & Confirmed \\
\midrule
Lambda invocations & 3.0$\times$ & AWS/Lambda Invocations & 270/90 \\
DynamoDB writes & 2.0$\times$ & ConsumedWriteCapacityUnits & 180/90 \\
DynamoDB reads & 1.5$\times$ & ConsumedReadCapacityUnits & 135/90 \\
Kinesis records & 3.0$\times$ & IncomingRecords & 270/90 \\
CloudWatch puts & 3.0$\times$ & Custom namespace & 270/90 \\
\midrule
\textbf{Total downstream} & \textbf{9.5$\times$} & & \\
\bottomrule
\end{tabular}
\caption{Measured amplification from a 3-function contact flow (routing + IVR fulfillment + post-call analytics). Two runs produced identical results. All values confirmed via CloudWatch.}
...
\caption{Left: throttle rate increases non-linearly as concurrency exceeds the cap. Right: downstream operations lost (red) vs produced (green). At 200 concurrent (40x cap), 86\% of calls produce zero downstream operations.}
\label{fig:cascade}
\end{figure}
...
\subsection{Cross-Domain Validation: When Does the GNN Add Value?}

We generated 250 real traces from our AWS contact flow to test when the GNN outperforms the static model:
...
\midrule
Fixed (no branching, 100 traces) & 0.000 & 0\% & No --- static is exact \\
Branching (60\% IVR, 150 traces) & 0.207 & 17--34\% & Yes --- reduces error \\
Alibaba production (4,976 traces) & $>$1.0 & 200--1000\% & Essential \\
\bottomrule
\end{tabular}
\caption{Fan-out variability determines when ML adds value. All AWS measurements confirmed by CloudWatch (391 invocations for branching test).}
...
\caption{Left: coefficient of variation increases with flow complexity. Right: model recommendation by complexity level. The GNN adds value only when conditional branching creates variable fan-out.}
\label{fig:crossdomain}
\end{figure}

The finding is a gradient: for fixed pipelines, the static amplification model (9.5$\times$) is exact. As branching increases, the static model's error grows and the GNN's ability to predict per-request fan-out becomes valuable. In production systems with complex conditional logic (Alibaba: CV$>$1.0), the GNN is essential---a static model is wrong by 2--10$\times$ on individual requests.

We validated the complete pipeline on live AWS infrastructure:
\begin{enumerate}
  \item \texttt{collect\_utilization.py} pulled 7 CloudWatch metrics from a live AWS environment
...
\textbf{Three-function topology.} Our measured amplification (9.5$\times$) comes from a 3-function contact flow (routing, IVR fulfillment, post-call analytics). Production contact centers may have 5--15 functions per call with additional services (transcription, sentiment analysis, outbound campaigns). Our measurement represents a mid-complexity topology; the most complex deployments will have higher amplification.
...
\textbf{GNN value depends on flow complexity.} For fixed contact flows (no branching), the static model is exact and the GNN adds no value. The GNN's advantage emerges with conditional branching (CV=0.207 at 60/40 IVR split) and becomes essential for complex production services (CV$>$1.0). Deployments with simple, linear flows should use the static model directly.
...
We demonstrated three findings through direct measurement: (1) a 3-function contact flow produces 9.5$\times$ downstream operations per call (confirmed by CloudWatch across two independent runs), (2) throttling produces binary failure with 84\% data loss, and (3) fan-out variability in production (CV$>$1.0) makes static models inadequate. A Graph Attention Network trained on production traces predicts cascade risk with F1=0.971, enabling proactive quota management before planned capacity events.
